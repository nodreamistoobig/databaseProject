...
<div class="content">
	...
	@enderror
	Название: <input class="@error('name')is-invalid @enderror" name="name" value="{{old('name')}}">
	</div>
		...
		<select class="@error('room_id')is-invalid @enderror" name="room_id">
			...
			@foreach ($departments as $department)
				@foreach ($department->rooms as $room)
					<option value="{{$room->id}}" @if(old('room_id')== $room->id) selected @endif>{{$department->name}} - {{$room->name}}</option>
				@endforeach
			@endforeach
		</select>
		...
		<select class="@error('educator_id')is-invalid @enderror" name="educator_id">
			...
			@foreach ($departments as $department)
				@foreach ($department->employees as $employee)
					<option value="{{$employee->id}}" @if(old('educator_id')== $employee->id) selected @endif>{{$department->name}} - {{$employee->surname}} {{$employee->firstname}} {{$employee->fathername}}</option>
				@endforeach
			@endforeach
		</select>
	...
	<select class="@error('manager_id')is-invalid @enderror" name="manager_id">
		...
		@foreach ($employees as $employee)
			<option value="{{$employee->id}}" @if(old('manager_id')== $employee->id) selected @endif>{{$employee->surname}} {{$employee->firstname}} {{$employee->fathername}}</option>
		@endforeach
	</select>
	</div>
